(function(window, undefined) {
					...
					if (list) {
						jQuery.each(arguments, function(_, arg) {
							...
							while ((index = jQuery.inArray(arg, list, index)) > -1) {
								list.splice(index, 1);
								if (firing) {
									if (index <= firinglength) { firinglength-- } if (index <="firingIndex)" firingindex-- }) return this }, has: function(fn) fn ? jquery.inarray(fn, list)> -1 : !! (list && list.length)
				},
				empty: function() {
					list = [];
					firingLength = 0;
	...
	var rxhtmlTag = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/gi,
		rtagName = /<([\w:]+) , rhtml="/<|&#?\w+;/," rnoinnerhtml="/<(?:script|style|link)/i," manipulation_rcheckabletype="/^(?:checkbox|radio)$/i," rchecked="/checked\s*(?:[^=]|=\s*.checked.)/i," rscripttype="/^$|\/(?:java|ecma)script/i," rscripttypemasked="/^true\/(.*)/," rcleanscript="/^\s*<!(?:\[CDATA\[|--)|(?:\]\]|--)">\s*$/g,
		wrapMap = {
	...
	jQuery.fn.extend({
		...
		domManip: function(args, table, callback) {
			...
			var fragment, first, scripts, hasScripts, node, doc, i = 0,
				...
				value = args[0],
				isFunction = jQuery.isFunction(value);
			if (isFunction || !(l <= 1 || typeof value !="=" "string" jquery.support.checkclone !rchecked.test(value))) { return this.each(function(index) var self="set.eq(index);" if (isfunction) args[0]="value.call(this," index, table ? self.html() : undefined) } self.dommanip(args, table, callback) }) (l) fragment="jQuery.buildFragment(args," this[0].ownerdocument, false, this); first="fragment.firstChild;" (fragment.childnodes.length="==" 1) (first) && jquery.nodename(first, "tr"); scripts="jQuery.map(getAll(fragment," "script"), disablescript); hasscripts="scripts.length;" for (; i < l; i++) node="fragment;" (i inoclone) true, true); (hasscripts) jquery.merge(scripts, getall(node, "script")) callback.call(table jquery.nodename(this[i], "table") findorappend(this[i], "tbody") this[i], node, i) doc="scripts[scripts.length" - 1].ownerdocument; jquery.map(scripts, restorescript); hasscripts; (rscripttype.test(node.type "") !data_priv.access(node, "globaleval") jquery.contains(doc, node)) (node.src) jquery.ajax({ url: node.src, type: "get", datatype: "script", async: global: "throws": true else jquery.globaleval(node.textcontent.replace(rcleanscript, "")) this }); jquery.each({ appendto: "append", prependto: "prepend", insertbefore: "before", insertafter: "after", replaceall: "replacewith" }, function(name, original) jquery.fn[name]="function(selector)" elems, ret="[]," insert="jQuery(selector)," last="insert.length" 1, elems="i" =="=" this.clone(true); jquery(insert[i])[original](elems); core_push.apply(ret, elems.get()) this.pushstack(ret) jquery.extend({ clone: function(elem, dataandevents, deepdataandevents) i, l, srcelements, destelements, clone="elem.cloneNode(true)," inpage="jQuery.contains(elem.ownerDocument," elem); (!jquery.support.noclonechecked (elem.nodetype="==" elem.nodetype="==" 11) !jquery.isxmldoc(elem)) destelements="getAll(clone);" srcelements="getAll(elem);" l="srcElements.length;" fixinput(srcelements[i], destelements[i]) (dataandevents) (deepdataandevents) getall(elem); getall(clone); clonecopyevent(srcelements[i], clonecopyevent(elem, clone) "script"); (destelements.length> 0) {
	...
	function getWidthOrHeight(elem, name, extra) {
		...
		if (val <= 0 || val="=" null) { name, styles); if (val < } (rnumnonpx.test(val)) return valueisborderbox="isBorderBox" && (jquery.support.boxsizingreliable elem.style[name]); + augmentwidthorheight(elem, extra (isborderbox ? "border" : "content"), valueisborderbox, styles)) "px" function css_defaultdisplay(nodename) var doc="document," display="elemdisplay[nodeName];" (!display) doc); (display="==" "none" !display) iframe="(iframe" jquery("<iframe frameborder="0" width="0" height="0">").css("cssText", "display:block !important")).appendTo(doc.documentElement);
	...
	if (jQuery.expr && jQuery.expr.filters) {
		jQuery.expr.filters.hidden = function(elem) {
			return elem.offsetWidth <= 0 && elem.offsetheight <="0" }; jquery.expr.filters.visible="function(elem)" { return !jquery.expr.filters.hidden(elem) } jquery.each({ margin: "", padding: border: "width" }, function(prefix, suffix) jquery.csshooks[prefix + suffix]="{" expand: function(value) var i="0," expanded="{}," parts="typeof" value="==" "string" ? value.split(" ") : [value]; for (; 4; i++) expanded[prefix cssexpand[i] || parts[i - 2] parts[0] if (!rmargin.test(prefix)) suffix].set="setPositiveNumber" }); r20="/%20/g," rbracket="/\[\]$/," rcrlf="/\r?\n/g," rsubmittertypes="/^(?:submit|button|image|reset|file)$/i," rsubmittable="/^(?:input|select|textarea|keygen)/i;" jquery.fn.extend({ serialize: function() jquery.param(this.serializearray()) serializearray: this.map(function() elements="jQuery.prop(this," "elements"); jquery.makearray(elements) this }).filter(function() type="this.type;" this.name !jquery(this).is(":disabled") rsubmittable.test(this.nodename) !rsubmittertypes.test(type) (this.checked !manipulation_rcheckabletype.test(type)) }).map(function(i, elem) val="jQuery(this).val();" null jquery.isarray(val) jquery.map(val, function(val) name: elem.name, value: val.replace(rcrlf, "\r\n") }) }).get() jquery.param="function(a," traditional) prefix, s="[]," add="function(key," value) value() (value="=" "" value); s[s.length]="encodeURIComponent(key)" "=" + encodeURIComponent(value)
			};
		...
		return s.join(" &").replace(r20, "+") function buildparams(prefix, obj, traditional, add) name; (jquery.isarray(obj)) jquery.each(obj, function(i, v) (traditional rbracket.test(prefix)) add(prefix, else buildparams(prefix "[" (typeof v="==" "object" "") "]", v, (!traditional jquery.type(obj)="==" "object") (name in obj) name obj[name], jquery.each(("blur focus focusin focusout load resize scroll unload click dblclick "mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave "change select submit keydown keypress keyup error contextmenu").split(" "), name) jquery.fn[name]="function(data," fn) arguments.length> 0 ? this.on(name, null, data, fn) : this.trigger(name)
		}
	});
...
$(function(d) {
 ...
 $(document).on("click",".toggle",function() {
		...
		H.toggleClass("collapsed");
		return false
	});</script></div></=></body></html></=></$1></=></$1></([\w:]+)></(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^></=></"></\></(\w+)\s*\></[\w\w]+>